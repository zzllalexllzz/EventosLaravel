    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between">
                <div >
                    <form action="/evento/fecha" method="POST">
                        @csrf
                        <!-- buscar por fecha -->
                ...
                <div>
                    <form action="/evento/ciudad" method="POST">
                        @csrf
                        <!-- buscar por ciudad -->
                ...
                <div>
                    <form action="/evento/categoria" method="POST">
                        @csrf
                        <!-- bucar por categoria -->
                        <x-input-label for="categoria" :value="__('Categoria')" />
                        <select class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full" id="categoria"  name="categoria" type="text" placeholder="" required>
    ...
    <div class="">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="text-center mb-5">
                <a href="/evento/nuevo" class=""><x-primary-button>Añadir Evento +</x-primary-button> 
                </a>
            </div>
                    ...
                    <table class="text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600">
                                <form method="POST" action='/evento/{{ $evento->id }}/update' enctype="multipart/form-data">
                                    @csrf
                                ...
                                <td class="px-4 py-2 text-right">
                                    @if (Auth::user()->rol=='admin' || Auth::user()->id==$evento->user_id)
                                    <a href="/evento/{{ $evento->id }}/borrar" class="font-medium text-blue-600 dark:text-blue-500 hover:underline"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                        </svg>
                                        </a>
                                        <a href="/evento/{{ $evento->id }}/detalle" class="font-medium text-blue-600 dark:text-blue-500 hover:underline"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
